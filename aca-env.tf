...
resource "azurerm_container_app_environment" "mlchain-aca-env" {
  name                       = var.aca-env
  location                   = azurerm_resource_group.rg.location
  resource_group_name        = azurerm_resource_group.rg.name
...
resource "azurerm_container_app_environment_storage" "nginxfileshare" {
  name                         = "nginxshare"
  container_app_environment_id = azurerm_container_app_environment.mlchain-aca-env.id
...
resource "azurerm_container_app_environment_certificate" "mlchaincerts" {
  name                         = "mlchaincerts"
  container_app_environment_id = azurerm_container_app_environment.mlchain-aca-env.id
...
resource "azurerm_container_app" "nginx" {
  ...
  ingress {
    ...
    custom_domain {
      name = var.aca-mlchain-customer-domain
      certificate_id = azurerm_container_app_environment_certificate.mlchaincerts.id
    }
  }
}
...
resource "azurerm_container_app" "sandbox" {
  ...
  template {
    ...
    container {
      name   = "mlchain"
      image  = var.mlchain-sandbox-image
      cpu    = 0.5
      memory = "1Gi"
      env {
        name  = "API_KEY"
...
resource "azurerm_container_app" "worker" {
  ...
  template {
    ...
    container {
      ...
      env {
        name  = "DB_DATABASE"
        value = azurerm_postgresql_flexible_server_database.mlchainpgsqldb.name
      }
      env {
        name  = "REDIS_HOST"
...
resource "azurerm_container_app" "web" {
  ...
  template {
    ...
    container {
      name   = "mlchain"
      image  = "mlchain/mlchain-web:0.6.11"
      cpu    = 1
      memory = "2Gi"
       env {
        name  = "CONSOLE_API_URL"
